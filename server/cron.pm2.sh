...
declare -a COMMAND_ARR=(
	"GATE_CASH_START_ALL"
	"GATE_CASH_SERVER_PROD"
	"GATE_CASH_SERVER_DEV"
	"GATE_CASH_DELETE_ALL"
)
...
if [[ "${1}" == "--cron" && "${2}" == "${COMMAND_ARR[0]}" ]]; then
	...
	{
		echo '[WARN] Make sure you build before run this code'
		createServerCRON "${COMMAND_ARR[1]}" "0 22 * * *" "dist/index.js"
	}

elif [[ "${1}" == "--cron" && "${2}" == "${COMMAND_ARR[2]}" ]]; then
	{
		echo '[WARN] Make sure you build before run this code'
		createServerCRON "${COMMAND_ARR[2]}" "0 22 * * *" "dist/index.js"
	}

# elif [[ "${1}" == "--start" && "${2}" == "${COMMAND_ARR[2]}" ]]; then
# 	{
# 		npm run cli update-balance >>"$LOG_DIR/${2}_$DATE.log" 2>"$LOG_DIR/${2}_$DATE.error.log"
# 	}
